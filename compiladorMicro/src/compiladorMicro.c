...
typedef enum
{
 INICIO, FIN, LEER, ESCRIBIR, ID, CONSTANTE, PARENIZQUIERDO, PARENDERECHO, PUNTOYCOMA, COMA, ASIGNACION, SUMA, RESTA, FDT, ERRORLEXICO,MULTIPLICACION
} TOKEN;
typedef struct
{
...
REG_EXPRESION GenInfijo(REG_EXPRESION e1, char * op, REG_EXPRESION e2)
{
 ...
 if ( op[0] == '*' ) strcpy(cadOp, "Multiplicar");
...
void Match(TOKEN t)
{
 if ( !(t == ProximoToken()) ) ErrorSintactico();//si no token t es igual a proximo token  hay error sintactico
 flagToken = 0;
}
...
TOKEN scanner()
{
 ...
 switch ( estado )
 {
  ...
  case 15 : return MULTIPLICACION; // Nuevo caso para la multiplicación
